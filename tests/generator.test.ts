...
describe('generateDailyLog', () => {
  ...
  it('should generate a daily log with a single commit', () => {
    ...
    // Check header (uses today's date, not commit date)
    expect(result).toContain('# Daily Log -')
    ...
    // Check work completed (now includes timestamps)
    ...
    expect(result).toContain('✏️') // modified emoji
  ...
  it('should generate a daily log with multiple commits', () => {
    ...
    // Check focus area inference (fix takes priority over refactor)
